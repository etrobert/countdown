 ...
 *  fight, with pre-damage hp and the cell each stood on: a dead clasher and a
 *  spent raider are already absent from the resolved board, so the animation
 *  replays them from here rather than reading them off it.
 *  - `clash`: two enemy minions traded blows — both lunge, either may die.
 *  - `mill`: a minion reached the enemy face and struck their deck, then left;
 *    `targetSeat` is the seat whose deck it hit, so the UI can aim the dash. */
...
function stepMinion(state: GameState, uid: number): StepResult {
  ...
  if (ahead < 0 || ahead >= LANE_CELLS) {
    ...
    return { state: raid(state, minion), event: { kind: "mill", attacker: minion, targetSeat } };
  }
  const other = minionAt(state, minion.lane, ahead);
  if (!other) return { state: advance(state, minion, ahead) };
  if (other.owner !== minion.owner)
    return { state: clash(state, minion, other), event: { kind: "clash", a: minion, b: other } };
  return { state };
}
 ...
 *  Returns the resolved state alongside the combat events that landed, in the
 *  order they resolved, so a caller can animate the blows before showing the
 *  outcome. */
