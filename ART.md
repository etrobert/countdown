...
| 2   | Frame          | **6:5 landscape** (6 × 5 cm), one uniform cell for every character                         |
| 3   | Card crop      | **None** — the source drawing _is_ the card vignette                                       |
| 4   | Support        | **One 18×30 cm sheet**, ~15 drawings in a 3×5 grid, **cell = 6 × 5 cm** (w×h)               |
...
| 9   | Slicing        | **By eye** — manual crop, aspect ratio locked to 6:5                                        |
...
- Slice **by eye**: crop each drawing with the ratio **locked to 6:5**, else the
  cards won't share a frame.
- Rendered in-app with `mix-blend-mode: multiply` so the paper drops out.

## Deferred

Export size per sliced image (suggest ~**600 × 500 px**, i.e. 6:5) — set once the
